...
// Get active KDS Orders (pending, preparing, ready, and recently served)
export const getKdsOrders = async (req: AuthRequest, res: Response) => {
  try {
...
// Update Order Preparation Status (KDS / Barista status lifecycle)
export const updateOrderStatus = async (req: AuthRequest, res: Response) => {
  try {
    ...
    io.emit("displayUpdate", { type: "status", orderId: order._id, status: order.status });
    const summary = await getTodayOrderSummary();
    io.emit("orderSummaryUpdate", summary);

    await ActivityLog.create({
      user: req.user?.id,
      action: `Status Transition for Order #${order.customOrderID}: [${prevStatus} -> ${order.status}]`,
      category: "order",
